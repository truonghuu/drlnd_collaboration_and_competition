...
The parameters of the local actor network are updated similarly (with backpropagation approach) but using a different loss function, which is the mean of the value given by the local Critic network for the actions taken by the local Actor network. We seek to maximize this quantity.
